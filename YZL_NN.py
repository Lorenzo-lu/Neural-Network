# -*- coding: utf-8 -*-
"""
Created on Sun Mar 15 21:57:56 2020
...
class YZL_NN:
    ...
    ## func 1:
    def __init__(self, X, Target, layer_nodes, W_init = 'Random', output_format = 'Softmax'):
        
        print('support three outputs format : 1. Softmax (for multiple classfification) 2. Sigmoid 3.Linear(for regression)!');
        ...
        self.Activation_map = {1:'Sigmoid',2:'ReLU',3:'Leaky_ReLU',4:'Tanh',5:'Softmax',6:'Linear'};
        ...
        if output_format == 'Softmax':
            ...
            if layer_nodes[-1] <= 1:
                print("Fatal Error: The last layer should be no fewer than 2! Or you should choose 'Sigmoid' or 'Linear'");
            else:
            ...
            if layer_nodes[-1] != 1:
                print("Warning: The last layer has wrong nodes number. Automatically convert to 1!");
                layer_nodes[-1] = 1;
            self.kinds = 1;
    ...
    ## func 2 and 3:  
    def Show_layers_act(self, L): # print the layers and their corresponding activation functions
        acts = [];
        for i in range(L):
    ...
    def Activation_sequence(self):
        ...
        while Judge == 'N':
            ...
            if Judge == 'N':
                print('Please input the index of the layer and the activation you want::\n e.g. [[1,2],[4,1]], means change the second layer\
    ...
    def Y2indicator(self,y,K): 
        ...
        return ind;
    ## func 5
    def Classification_rate(self,T,Y,last_layer_act):   
    ...
    ## func 7
    def Forward(self,L,X,W,b,layer_activation):
    ...
    ## func 8
    def Derivative(self,L,Target,nodes,W,layer_activation):
        L = L - 1; 
    ...
    ## func 9
    def Cost(self,T,Y,last_layer_act):
    ...
    def Optimal(self,learning_rate = 1e-5,converge = 1e-7 ,beta = 0.0,regularization = 0,plot_step = 1000):
        if beta < 0 or beta > 1:
        ...
        while abs(c - c0) >  converge:            
            c0 = c;            
            self.nodes = self.Forward(self.L+1,self.nodes[0],self.W,self.b, self.layer_activation);            
            Y = self.nodes[-1];  
            c = self.Cost(self.Target,Y, self.layer_activation[-1]);
            if epoch%(plot_step) == 0:
                r = self.Classification_rate(self.Target,Y, self.layer_activation[-1]);
                if r:
            ...
            delta_W,delta_b = self.Derivative(self.L+1, self.Target, self.nodes, self.W, self.layer_activation);
            for i in range(self.L):
                if type(VdW[i]) == bool:
                    VdW[i] = delta_W[i]; 
                if type(Vdb[i]) == bool:
